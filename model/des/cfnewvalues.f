...
! Define movable particles (in contrast to particle wall)
...
!$omp reduction(.or.:do_nsearch) schedule (auto)                      	  
      DO L = 1, MAX_PIP
!
! Prevent the still particles into the loop
!	DO L = 1, N_LIVE_PARTICLES
! pradeep skip ghost particles
!!         if(pc.gt.pip) exit

! Prevent the particle wall from getting into the loop
...
! first-order method
! special treatment for virtual wall particles - Wesley
        if (RO_SOL(L).GE.RO_WALL) then
