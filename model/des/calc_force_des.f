...
!           by Wesley Xu: add glass stress-strain record Date: 06/21/13 C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
...
!  Define particle wall density for glass simulation-Wesley
   Double Precision :: Ro_wall=3.D0
!   LOGICAL :: SSOPEN=.false.


! Wesley- local values used for calculating bonding forces
      DOUBLE PRECISION DISTAPART_T, DIST_ARM, BOND_N, BOND_T
      DOUBLE PRECISION FORCE_COH_T(DIMN), FORCE_COH_N(DIMN) 
...
! Wesley- initialize the total force for glass project
      FXL = ZERO
      FYL = ZERO
      FXR = ZERO
      FYR = ZERO
...
! WESLEY INITIALIZE BONDING FORCE
        FORCE_COH_N(:) = ZERO
        FORCE_COH_T(:) = ZERO
	ENDIF
...
! Wesley - Comment out to prevent VDW calculation between particle and wall (12/03/2013)
...
! Save the tangential displacement history with the correction of Coulomb's law
 ...
 200           CONTINUE
!
! Call Attrition model after completion of particle-wall calculation - WESLEY FOR GLASS 050114


                  IF(DES_ATTRITION) THEN
		    CALL CALC_ATTRITION_DES(LL,FN(LL,:),tangent(:),overlap_t, des_Radius(LL))                 
                  ENDIF  
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Wesley - MODIFIED VDW MODEL TO REPRESENT THE LINEAR BONDING STRESS (04/02/2014)   
		  ...
		  IF(USE_COHESION .AND. VAN_DER_WAALS) THEN
		     ...
		     IF (LINKS(LL,1).GT.0.AND.NEIGHBOURS(LL,1).GT.0) THEN

            	       DO KK=1, LINKS(LL,1)
                        IF (I.EQ.LINKS(LL, KK+1)) THEN 

! CALCULATE NORMAL BOND FORCE     
		            DISTAPART = (R_LM-DISTMOD) ! DISTANCE BETWEEN PARTICLE SURFACE
                            NORMAL = DIST(:)/DISTMOD   ! NORMAL DIRECTION VECTOR
			     FORCE_COH = -HAMAKER_CONSTANT * DISTAPART



! CALCULATE TANGENTIAL BOND FORCE
! CALCULATE RELATIVE VELOCITY FOR THE BOND PAIR
                         CALL CFRELVEL(LL, I, V_REL_TRANS_NORM, &
                          V_REL_TRANS_TANG, TANGENT, NORMAL, DISTMOD)  
			    DISTAPART_T = V_REL_TRANS_TANG*DTSOLID


! CALCULATE THE TANGENTIAL DISPLACEMENT FOLLOWING VAN DER HOEF ET AL. (2006) ONLY FOR 2D
                  	    SIGMAT_OLD(:) = PFT(LL,KK+1,:)                                        
                  	    NORM_OLD(:) =  PFN(LL,KK+1,:)
                  	    TANG_OLD(1) = -NORM_OLD(2)
                         TANG_OLD(2) = NORM_OLD(1)
                         TANG_NEW(1) = -NORMAL(2)
                         TANG_NEW(2) = NORMAL(1)
                         SIGMAT(:)=DES_DOTPRDCT(SIGMAT_OLD, TANG_OLD)*TANG_NEW(:)
                         SIGMAT(:)=SIGMAT(:)+DISTAPART_T*TANGENT(:)
                         PFT_TMP(:) = SIGMAT(:)
                         FORCE_COH_T(:) = -WALL_HAMAKER_CONSTANT * PFT_TMP(:)
         
                         
! CALCULATE MAXIMUM NORMAL AND TANGENTIAL BOND STRESS
			    BOND_T = SQRT(DES_DOTPRDCT(FORCE_COH_T, FORCE_COH_T))/(MIN(DES_R_L, DES_R_I)**2.D0*2.D0*ACOS(0.D0))

                         IF (R_LM.LE.DISTMOD) THEN
				BOND_N = 12.D0*SQRT(DES_DOTPRDCT(FORCE_COH_T, FORCE_COH_T))*DES_R_L &
					  / (3*MIN(DES_R_L, DES_R_I)**2.D0+DES_R_L**2.D0)
                         ELSE
                            DIST_ARM = DISTMOD-(DISTMOD**2+DES_R_L**2-DES_R_I**2)/(2.D0*DISTMOD)
				BOND_N = 12.D0*SQRT(DES_DOTPRDCT(FORCE_COH_T, FORCE_COH_T))*DIST_ARM &
					  / (3*MIN(DES_R_L, DES_R_I)**2.D0+DIST_ARM**2.D0) 
                         ENDIF


                         BOND_N = BOND_N + FORCE_COH/(MIN(DES_R_L, DES_R_I)**2.D0*2.D0*ACOS(0.D0))

                         
! DETERMINE WHEHTER FAILURE CRITERIA HAS BEEN REACHED ON NORMAL OR TANGENT                                    										                                                 
                         IF (ABS(BOND_N).GE.VDW_OUTER_CUTOFF.OR.ABS(BOND_T).GE.VDW_INNER_CUTOFF) THEN
				 FORCE_COH = ZERO
                             FORCE_COH_T(:) = ZERO
                             CALL REMOVE_PART_FROM_LINK_LIST(LL, I)
                         ENDIF 
                         EXIT
                        ENDIF
                      ENDDO
                   ...
                   ENDIF ! IF (LINKS(LL,1).GT.0)


                  DO IDIM=1, DIMN
                     NORM_DIST = DIST(IDIM)/DISTMOD
                     FORCE_COH_N(IDIM) = NORM_DIST*FORCE_COH
                     FCOHESIVE(LL, IDIM) = FCOHESIVE(LL, IDIM) + FORCE_COH_N(IDIM) + FORCE_COH_T(IDIM)
                  ENDDO  
 ! FOR DEBUG ONLY - WESLEY
 !                 IF (LL.EQ.9.AND.I.EQ.10) WRITE(*,*) S_TIME, FORCE_COH, FORCE_COH_T(1), FORCE_COH_T(2)

! UPDATE TOTAL FORCE AND TOW
                 FN(LL,:) = FORCE_COH_N(:)
                 FT(LL,:) = FORCE_COH_T(:)
                 IF (R_LM.LE.DISTMOD) THEN
                    CALL CFFCTOW(LL, I, NORMAL, R_LM) 
                 ELSE
                    CALL CFFCTOW(LL, I, NORMAL, DISTMOD)
                 ENDIF 
 

		  ENDIF ! IF(USE_COHESION .AND. VAN_DER_WAALS)   
...
! Calculate the total force   and TOW on a particle in a particle-particle collision
                  CALL CFFCTOW(LL, I, NORMAL, DISTMOD)
...
!
! Call Attrition model after completion of particle-particle calculation WELSEY FOR GLASS 050114
!
...
! Wesley calculate the total force on each of the wall particles
! Left wall

        IF (ABS(RO_SOL(LL)-RO_WALL).LT.SMALL_NUMBER) THEN
              FXL = FXL + FN(LL,1) + FT(LL,1)
              FYL = FYL + FN(LL,2) + FT(LL,2)

! Right wall
            ELSE IF (ABS(RO_SOL(LL)-RO_WALL-1.D-1).LT.SMALL_NUMBER) THEN
              FXR = FXR + FN(LL,1) + FT(LL,1)
              FYR = FYR + FN(LL,2) + FT(LL,2)
     
! Top wall
            ELSE IF (ABS(RO_SOL(LL)-RO_WALL-2.D-1).LT.SMALL_NUMBER) THEN
              FXT = FXT + FN(LL,1) + FT(LL,1)
              FYT = FYT + FN(LL,2) + FT(LL,2)
         
! Bottom wall
             ELSE IF (ABS(RO_SOL(LL)-RO_WALL-3.D-1).LT.SMALL_NUMBER) THEN
               FXB = FXB + FN(LL,1) + FT(LL,1)
               FYB = FYB + FN(LL,2) + FT(LL,2)

! NOT pseudo particle wall
             ELSE
       ENDIF
...
!
! WESLEY USE POSTCOHESIVE TO INDICATE WHETHER THERE IS BOND OR NOT

     IF (LINKS(LL,1).GT.0) POSTCOHESIVE(LL)=1.D0

! FOR DEBUG ONLY - WESLEY
!     IF (LINKS(LL,1).GT.0.AND.S_TIME.GT.0.29) WRITE(*,*) LL, LINKS(LL,1), S_TIME
