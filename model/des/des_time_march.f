...
!
!     Reviewer: Wesley Xu, Dave Decroix                  Date: 04-April-14
!     Revision: Reactivated LINKS to represent inter-particle bonding
! ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
...
! Wesley use module for glass project
      USE parallel
      USE cdist
      USE mpi_utility
...
! Local variables
!------------------------------------------------
! WESLEY - EXPAND THE RANGE OF INTEGER SPECIFICALLY FOR FACTOR
      INTEGER(KIND=SELECTED_INT_KIND(15)):: NN, FACTOR, NP, IJK, I, J, K, BCV_I, LL, PC
...
! Wesley define output data file name and status for glass project

      Character*50, parameter :: SSNAME_DATA = "Monitor_Stress.dat"
      Logical :: FStatus

! Wesley define writing format for glass project

      Character (len=*), parameter :: fmt="(ES12.3, 8ES12.3)"

! WESLEY DEFINE PSUEDO-PARTITICLE DENSITY  04/04/2014
...
! Initialize attrition radius array desradiusnew, not suitable for MPPIC - Wesley 04/03/14
!         Do NP=1, MAX_PIP
...
! Wesley removed the ".NOT.VAN_DER_WAALS" from the following if statement
! Here initialize used to establish the links
...
! Wesley add to establish the initial bonds
               IF (USE_COHESION) THEN
                 CALL NEIGHBOUR
           	   DO NP=1, MAX_PIP
                   IF (RO_SOL(NP).LT.RO_PWALL) THEN
		      IF (NEIGHBOURS(NP, 1).GT.0) THEN

		       DO I=1, NEIGHBOURS(NP,1)
                      IF (RO_SOL(NEIGHBOURS(NP, I+1)).LT.RO_PWALL.AND.NEIGHBOURS(NP, I+1).GT.NP) THEN ! avoid double count of links
                          CALL ADD_PART_TO_LINK_LIST(NP, NEIGHBOURS(NP, I+1))
                      ENDIF	      
                     ENDDO
                    ENDIF
                   ENDIF
...
! pradeep following is added  to avoid error with check compile flag 
 ...
 1999    FORMAT(/1X,70('- '),//,10X,  & 
    ...
    ELSE      
         ...
         FACTOR = CEILING(real((TSTOP-TIME)/DTSOLID), SELECTED_INT_KIND(15))
...
! JFD : Update radius  - Wesley DEACTIVATED 04/03/2014
!         Do NP=1, MAX_PIP
!           DESRadiusNew(NP)=DES_RADIUS(NP)
!         ENDDO
...
!---------------------------------------------------------------------------------------------------------------
! Wesley write total force output for glass project
! only write one line after every   
          Call global_all_sum(FXL)
          Call global_all_sum(FYL)
          Call global_all_sum(FXR)
          Call global_all_sum(FYR)
          Call global_all_sum(FXT)
          Call global_all_sum(FYT)
          Call global_all_sum(FXB)
          Call global_all_sum(FYB)

          if (BDIST_IO) then
            inquire(file=SSNAME_DATA, EXIST=FSTATUS)
            IF (FSTATUS) THEN
              open (unit=373, file=SSNAME_DATA, status="old", position="append", action="write")
             else
              open (unit=373, file=SSNAME_DATA, status="new", action="write")
            ENDIF

          else if (mype.eq.pe_io) then
            inquire(file=SSNAME_DATA, EXIST=FSTATUS)
            IF (FSTATUS) THEN
              open (unit=373, file=SSNAME_DATA, status="old", position="append", action="write")
            else
          ...
          if (S_Time.le.DTSOLID) THEN
             write(373,fmt) zero,zero,zero,zero,zero,zero,zero,zero,zero
          else
             write(373,fmt) S_TIME,FXL,FYL,FXR,FYR,FXT,FYT,FXB,FYB
          endif
          close(373)
